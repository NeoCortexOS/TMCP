...
string stripExpression(string anim)
{
    list l=llParseString2List(anim, ["::"], []);
    return llList2String(l,0);
}

stopAnim() {
    if (hasPerms) if (currentAnim)
        llStopAnimation(stripExpression(currentAnim));
}

runAnim() {
    if (hasPerms) if (currentAnim)
        llStartAnimation(stripExpression(currentAnim));
}
